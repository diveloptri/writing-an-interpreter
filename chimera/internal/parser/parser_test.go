...
func TestIdentifierExpression(t *testing.T) {
	...
	if !ok {
		t.Fatalf("program.Statements[0] is not ast.ExpressionStatemt. got=%T",
	...
	if ident.Value != "foobar" {
		t.Errorf("ident.Value not %s. got=%T",
			"foobar", ident.Value)
	}
...
func TestParsingPrefixExpressions(t *testing.T) {
	...
	for _, tt := range prefixTests {
		...
		if exp.Operator != tt.operator {
			t.Fatalf("exp.Operator os not '%s'. got=%s", tt.operator, exp.Operator)
		}
